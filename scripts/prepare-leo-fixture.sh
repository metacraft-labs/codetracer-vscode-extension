...
if ! command -v cargo >/dev/null 2>&1; then
  ...
  echo "  Install the Rust toolchain or enter nix develop in $LEO_RECORDER_DIR."
  exit 1
fi

# Build the recorder binary.
echo "Building codetracer-leo-recorder..."
if command -v direnv >/dev/null 2>&1 && [ -f "$LEO_RECORDER_DIR/.envrc" ]; then
  direnv exec "$LEO_RECORDER_DIR" cargo build --manifest-path "$LEO_RECORDER_DIR/Cargo.toml"
else
  cargo build --manifest-path "$LEO_RECORDER_DIR/Cargo.toml"
fi
